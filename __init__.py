...
class UnloadTimeTracker:
        ...
        try:
            ...
            if new_value < 0:
                return aiohttp.web.Response(
                    status=400, text="Time remaining must be non-negative"
                )
            self.unload_delay_seconds = new_value
            return aiohttp.web.json_response(
                {"status": "success", "time_remaining": self.unload_delay_seconds}
            )
    ...
    def start_timer(self):
        ...
        self.timer_handle = PromptServer.instance.loop.create_task(check_and_free())

    def free(self):
        # For some reason, trying to construct a request and call PromptServer.instance.post_free
        # hangs. Might be because that requires making this method async, forcing us to
        # await in start_timer.check_and_free, and something something event loops.
        # Manually duplicating the code from post_free works.
